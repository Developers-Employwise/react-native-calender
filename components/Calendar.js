...
export default class Calendar extends Component {
  ...
  prepareCustomViewDates(customviewDates) {
	  
	const customView = {};

    if (customviewDates) {
      customviewDates.forEach(event => {
        if (event.date) {
  ...
  onWeekRowLayout = (event) => {
    if (this.state.rowHeight !== event.nativeEvent.layout.height && this.state.rowHeight < event.nativeEvent.layout.height) {
  ...
  renderCalendarView(calFormat, argMoment, eventsMap,customDayViewMap) {
    let renderIndex = 0,
        weekRows = [],
        days = [];
      ...
      if (dayIndex >= 0 && dayIndex < argDaysCount) {
        days.push((
          this.renderDay({
			...
			customDayView: customDayViewMap[thisMoment.format('YYYY-MM-DD')] ||
							customDayViewMap[thisMoment.format('YYYYMMDD')]
          })
        ));
      } else {
        days.push(
  ...
  render() {
	...
	const customDayViewMap  = this.prepareCustomViewDates(this.props.customDayViews);
      ...
      <View style={[styles.calendarContainer, this.props.customStyle.calendarContainer]}>
        ...
        {this.props.scrollEnabled ?
          <ScrollView
             ...
             >
              {calendarDates.map((date) => this.renderCalendarView(this.props.calendarFormat, moment(date), eventDatesMap,customDayViewMap))}
           </ScrollView>
         :
